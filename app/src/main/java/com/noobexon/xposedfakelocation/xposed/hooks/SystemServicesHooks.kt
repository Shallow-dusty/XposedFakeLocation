...
class SystemServicesHooks(
    ...
    private fun hookWifiServiceImpl(wifiServiceClass: Class<*>) {
        hookAll(wifiServiceClass, "getScanResults") { chain ->
            ...
            if (identity != null && shouldSpoofWifiArgs(chain.args, identity.targetApps)) {
                val fakeResults = createFakeScanResults(result, identity)
    ...
    private fun createFakeScanResults(original: Any?, identity: WifiIdentity): List<ScanResult> {
        ...
        return WifiScanResultPolicy.createSpecs(
            SpoofedWifiIdentity(
                ssid = identity.ssid,
                bssid = identity.bssid,
                rssi = identity.rssi
            )
        ).mapNotNull { it.toScanResult(template) }
    }
